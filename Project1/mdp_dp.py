...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-8):
    ...
    while True:
        ...
        for state in range(nS):
            ...
            mean_reward_actions = 0
            for action in range(nA):
                prob_action = policy[state][action]
                transitions = P[state][action]

                mean_reward_transitions = 0
                for transition in transitions:
                    future_state_probability = transition[0]
                    future_state = transition[1]
                    future_state_reward = transition[2]
                    discounted_reward = gamma * value_function[future_state]

                    # Probability of trans. x reward
                    mean_reward_transitions += future_state_probability * (future_state_reward + discounted_reward)

                mean_reward_actions += prob_action * mean_reward_transitions

            value_function[state] = mean_reward_actions
            accuracy = abs(v - mean_reward_actions)
            delta = max(delta, accuracy)
        if delta < tol:
            break
    ...
    for state in range(nS):
        ...
        for action in range(nA):
            ...
            for transition in transitions:
                ...
                future_state = transition[1]
                future_state_reward = transition[2]
                discounted_reward = gamma * value_from_policy[future_state]

                # Probability of trans. x reward
                mean_reward_transitions += future_state_probability * (future_state_reward + discounted_reward)

            if mean_reward_transitions > best_reward:
                best_reward = mean_reward_transitions
                best_action = action
        ...
        for state in range(nS):
            ...
            for action in range(nA):
                ...
                for transition in transitions:
                    ...
                    future_state = transition[1]
                    future_state_reward = transition[2]
                    discounted_reward = gamma * V_new[future_state]

                    # Probability of trans. x reward
                    mean_reward_transitions += future_state_probability * (future_state_reward + discounted_reward)

                if mean_reward_transitions > max_reward:
                    max_reward = mean_reward_transitions
                    best_action = action
